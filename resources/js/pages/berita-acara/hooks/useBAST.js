...
export const useBAST = (projekKerjaId = null) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing && editId) {
        await api.put(`/bast/${editId}`, submitData);
        alert(tr("Dokumen BAST berhasil diperbarui!", "BAST document updated successfully!"));
        setIsEditing(false);
        setEditId(null);
        setEditNomorSurat("");
        resetForm();
  ...
  const handleEdit = async (item) => {
    try {
      const response = await api.get(`/bast/${item.id}`);
      const data = response.data.data;
      setFormData(mapKlientDocToForm(data, "tanggal_bast", true));
      setEditId(item.id);
