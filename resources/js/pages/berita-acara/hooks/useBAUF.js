...
export const useBAUF = (projekKerjaId = null) => {
  ...
  const {
    ...
    handleSchedule,
  } = useDocumentSchedule(projekKerjaId, "bauf");

  const [formData, setFormData] = useState({
    nama_hari: "",
    tanggal_bauf: "",
    tanggal_bauf_display: "",
    nama_klient: "",
  ...
  const fetchNextNomorSurat = async () => {
    ...
    try {
      ...
      setNextNomorSurat("001/BAUF-HSR/I/2026");
    } finally {
  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      const saved = localStorage.getItem("bauf_history");
      if (saved) {
        try {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "tanggal_bauf") {
      setFormData(prev => ({
        ...prev,
        [name]: value,
        tanggal_bauf_display: formatDateToIndonesian(value),
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing && editId) {
        await api.put(`/bauf/${editId}`, submitData);
        alert(tr("Dokumen BAUF berhasil diperbarui!", "BAUF document updated successfully!"));
        setIsEditing(false);
        setEditId(null);
        setEditNomorSurat("");
        resetForm();
      ...
      link.href = url;
      link.setAttribute('download', 'BAUF-' + nextNomorSurat.replace(/\//g, '-') + '.pdf');
  ...
  const handleGeneratePDF = async (item) => {
    ...
    try {
      ...
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', 'BAUF-' + item.nomor_surat.replace(/\//g, '-') + '.pdf');
      document.body.appendChild(link);
      link.click();
      link.remove();
      window.URL.revokeObjectURL(url);

      alert(tr("PDF BAUF berhasil di-generate ulang!", "BAUF PDF regenerated successfully!"));
    } catch (error) {
      console.error("Error generating PDF:", error);
      alert(tr("Gagal generate PDF. Silakan coba lagi.", "Failed to generate PDF. Please try again."));
    } finally {
  ...
  const handleDelete = async (id) => {
    if (window.confirm(tr("Yakin ingin menghapus dokumen ini dari riwayat?", "Are you sure you want to delete this document from history?"))) {
      try {
        ...
        localStorage.setItem("bauf_history", JSON.stringify(updatedHistory));
  ...
  const handleEdit = async (item) => {
    try {
      const response = await api.get(`/bauf/${item.id}`);
      const data = response.data.data;
      setFormData(mapKlientDocToForm(data, "tanggal_bauf", true));
      setEditId(item.id);
