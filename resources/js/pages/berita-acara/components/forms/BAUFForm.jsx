...
import { Plus, Download, Trash2, FileText, Calendar, PenLine, Edit, X, Save } from "lucide-react";
...
export const BAUFForm = ({ 
  ...
  return (
    <div className="bg-white rounded-3xl shadow-md p-4 sm:p-6 lg:p-8">
      <form id="bauf-form" onSubmit={onSubmit}>
        {isEditing && (
        ...
        <div className="mb-8">
          <h3 className="text-lg font-semibold text-gray-700 mb-4 pb-2 border-b flex items-center gap-2">
            ...
            {isEditing ? tr("Edit Dokumen BAUF", "Edit BAUF Document") : tr("Informasi Dokumen", "Document Information")}
          </h3>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">{tr("Hari (Otomatis)", "Day (Automatic)")}</label>
              <input
                type="text"
                name="nama_hari"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">{tr("Tanggal Pelaksanaan", "Execution Date")} <span className="text-red-500">*</span></label>
              ...
              {formData.tanggal_bauf_display && (
                <p className="text-sm text-gray-600 mt-1">{tr("Format", "Format")}: {formData.tanggal_bauf_display}</p>
              )}
            </div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">{tr("Kota Tanda Tangan", "Signature City")} <span className="text-red-500">*</span></label>
              <input
                type="text"
                ...
                required
              />
              {/* <p className="text-xs text-gray-500 mt-1">{tr("Kota yang muncul sebelum tanggal tanda tangan di PDF", "City shown before the signature date in the PDF")}</p> */}
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">{tr("Nama Klien", "Client Name")} <span className="text-red-500">*</span></label>
              <input
                type="text"
                name="nama_klient"
            ...
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-gray-700 mb-2">{tr("Hasil Uji Fungsi", "Function Test Result")} <span className="text-red-500">*</span></label>
              <input
                type="text"
                name="hasil"
                ...
                required
              />
              <p className="text-xs text-gray-500 mt-1">{tr("Ketik hasil uji fungsi peralatan", "Enter function test result")}</p>
            </div>
          </div>
        </div>
            ...
            <div key={index} className="grid grid-cols-1 sm:grid-cols-12 gap-3 sm:gap-4 mb-4 items-end bg-gray-50 p-3 sm:p-4 rounded-xl">
              ...
              <div className="col-span-1 sm:col-span-1">
                {formData.items.length > 1 && (
                  <button type="button" onClick={() => onRemoveItem(index)} className="w-full sm:w-auto px-3 py-2.5 sm:py-3 bg-red-600 text-white rounded-xl hover:bg-red-700 transition flex items-center justify-center gap-2 sm:gap-0" title={tr("Hapus barang", "Delete item")}>
                    <Trash2 size={18} />
        ...
        <div className="flex flex-col sm:flex-row justify-end gap-3 sm:gap-4">
          <button type="button" onClick={onReset} className={`w-full sm:w-auto px-6 py-3 rounded-xl font-medium transition ${isEditing ? "text-orange-700 bg-orange-100 hover:bg-orange-200" : "border border-gray-300 text-gray-600 hover:bg-gray-50"}`}>
          ...
          <button type="submit" disabled={loading} className={'w-full sm:w-auto px-6 sm:px-8 py-3 rounded-xl font-medium flex items-center justify-center gap-2 transition-all duration-200 ' + (loading ? 'bg-gray-400 cursor-not-allowed text-white' : 'bg-blue-600 hover:bg-blue-700 text-white hover:shadow-lg')}>
            {loading ? (
              <><svg className="animate-spin h-5 w-5" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" /></svg>{isEditing ? tr("Menyimpan...", "Saving...") : "Generating..."}</>
            ) : isEditing ? (
              <><Save size={20} />{tr("Simpan Perubahan", "Save Changes")}</>
            ) : (
